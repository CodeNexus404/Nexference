...
export function registerCustomProviderRoutes(app) {
  ...
  app.get('/api/custom-providers/favicon', async (req, res) => {
    ...
    try {
      ...
      const resp = await fetch(u.origin, {
      ...
      if (iconMatch) {
        ...
        else if (href.startsWith('/')) href = u.origin + href;
        else if (!href.startsWith('http')) href = u.origin + '/' + href;
        return res.json({ ok: true, url: href });
      }
      return res.json({ ok: true, url: u.origin + '/favicon.ico' });
    } catch { return res.json({ ok: false, url: null }); }
  });
